(function() {
  var BuildESM, BuildMain, FindNames, compile, read, write;
  ...
  BuildMain = function() {
    ...
    return BuildESM(source);
  };

  BuildESM = function(source) {
    var lines, names;
    names = FindNames(source);
    lines = names.map(function(name) {
      return `export const ${name} = m.${name}`;
    });
    return write(`${LIB}/esm.mjs`, ["import m from './main.js'", ...lines].join("\n"));
  };
