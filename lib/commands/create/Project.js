// Generated by CoffeeScript 2.7.0
(function() {
  var ProjectFile;

  ({ProjectFile} = require('./ProjectFile'));

  require('./helpers');

  exports.Project = bow({
    init: {
      name: null,
      template: null,
      dir: null
    },
    setup: async function() {
      var code, compile, metafile, metafile_output, project, read;
      switch (this.template.type) {
        case 'simple':
          await this.create_files();
          break;
        case 'complex':
          ({compile} = require('coffeescript'));
          ({read} = IO.sync);
          ({metafile} = this.template);
          code = (await read(metafile));
          project = this;
          metafile_output = eval(compile(code, {
            bare: true
          }));
          if (typeof metafile_output === 'function') {
            metafile_output.call(this);
          }
          await this.create_files();
      }
      return this;
    },
    methods: {
      create_files: async function() {
        var i, len, path_to_project_file, ref, results, template_file;
        ref = this.template.files;
        results = [];
        for (i = 0, len = ref.length; i < len; i++) {
          template_file = ref[i];
          path_to_project_file = (await this.create_new_file_from(template_file));
          results.push(console.log(path_to_project_file));
        }
        return results;
      },
      create_new_file_from: async function(template_file) {
        var filename, output, project, project_file, text;
        project_file = ProjectFile(template_file.transpose_to(this.dir));
        if (template_file.is_evaluable()) {
          project = this;
          output = (await template_file.output_for({project}));
          if (text = output.text) {
            if (filename = output.filename) {
              project_file.change_filename_to(filename);
            }
          } else {
            text = output;
          }
          await project_file.write(text);
          if (output.executable) {
            await sh(`chmod +x ${project_file.path}`);
          }
        } else {
          await project_file.copy(template_file.path);
        }
        return project_file.path;
      }
    }
  });

}).call(this);
